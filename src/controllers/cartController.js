...
export const getCart = async (req, res) => {
  ...
  try {
    ...
    const ids = cart.products.map(({ productId }) => new ObjectId(productId))
...
export const postProductToCart = async (req, res) => {
  ...
  try {
    ...
    if (!cart) {
      ...
      if (quantityUpdate.value) return res.send("Product quantity updated")
...
export const deleteProductFromCart = async (req, res) => {
  ...
  try {
    ...
    await db
      .collection("carts")
      .updateOne({ userId: userId }, { $set: { products: [...newProducts] } })
